...
@Component({
  ...
  template: `
    <section class="mx-auto flex min-h-screen max-w-xl flex-col items-center justify-center gap-4 px-6 text-center">
      ...
      <p class="text-secondary-600 dark:text-dark-secondary-300">The server did not confirm whether you are still signed in. Retry before signing in again.</p>
      <p-button (onClick)="retry()" type="button" label="Retry" severity="secondary" />
      <a
